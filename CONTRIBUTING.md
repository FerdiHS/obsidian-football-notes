...
To merge a Release Please PR, first synchronize the branch and wait for every check to report success. Then [FerdiHS](https://github.com/FerdiHS) should apply the `release: ready` label. The automation validates the current head commit, removes stale labels after synchronization or conversion to draft, and removes the label with a comment when validation or merging fails; reapply it after fixing the issue. Repository auto-merge is intentionally disabled for this process. The workflow identifies the Release Please author using the `RELEASE_PLEASE_APP_SLUG` repository variable.
